...
$sql_num_stocks = "SELECT COUNT(*) AS count FROM Stocks WHERE 1=1";
if($termExists) {
    $sql_num_stocks = $sql_num_stocks . " AND companyName LIKE '%$searchTerm%' OR symbol LIKE '%$searchTerm%'";
}
$sql_num_stocks = $sql_num_stocks . ";";

// var_dump($sql_num_stocks);
...
<body class="bg-dark text-light">
    ...
    <main>
        ...
        <form class="form-inline d-flex justify-content-center" name="searchForm"
            action="stocksDark.php" method="GET" autocomplete="off">
            <!-- <div class="form-group mx-sm-3 mb-2"> -->
            <!-- <div class="form-row"> -->
            <div class="autocomplete">
                <input type="text" id="searchBar" class="form-control mx-sm-3 mb-3 w-auto mr-3" name="searchTerm"
                <?php if ($searchTerm): ?>
                ...
                <tbody>
                    <?php while($row = $results_stocks->fetch_assoc()): ?>
                        <tr class="priceData" data-symbol="<?php echo $row['symbol']; ?>">
                            ...
                            <td class="priceHere text-success" style="font-weight: bold;">$$$</td>
                            <td>
                                <!-- Button trigger modal -->
                                <button type="button" class="btn btn-secondary" data-toggle="modal" data-target="#<?php echo str_replace('%', '_', urlencode($row['symbol'])); ?>">...</button>
                                ...
                                <div class="modal fade" id="<?php echo str_replace('%', '_', urlencode($row['symbol'])); ?>" tabindex="-1" role="document" aria-labelledby="<?php echo str_replace('%', '_', urlencode($row['symbol'])); ?>CenterTitle" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                                        ...
                                        <div class="modal-body" style="font-weight: normal;">
                                            <div class="row">
                                                <div class="col curr-price priceHere text-success">
                                                    Curr Price
                                                </div>
            ...
            <ul class="pagination justify-content-center">
                <li class="page-item">
                    ...
                    <?php if(isset($_GET['searchTerm'])) {
                        ...
                        echo $page_url . "?searchTerm=\"\"" . $first_page;
                    } ?>">First</a>
                </li>
                <li class="page-item">
                    <a class="page-link" href="<?php if(isset($_GET['searchTerm'])) {
                ...
                <li class="page-item">
                    <a class="page-link" href="<?php if(isset($_GET['searchTerm'])) {
                        ...
                        echo $page_url . "?searchTerm=\"\"" . ($current_page + 1);
                    } ?>">Next</a>
                </li>
                <li class="page-item">
                    <a class="page-link" href="<?php if (isset($_GET['searchTerm'])) {
                        ...
                        echo $page_url . "?searchTerm=\"\"" . $last_page;
                    } ?>">Last</a>
                </li>
            </ul>
        </nav>
...
for(let i = 0; i < window.myStocks.length; i++) {
    $.ajax({
        url: "https://api.iextrading.com/1.0/stock/" + window.myStocks[i] + "/quote",
        success: function(result) {
            // console.log(result);
        ...
        for(let ind = 0; ind < window.myPrices.length; ind++) {
            if(window.myPrices[ind].symbol == divSymb) {
                ...
                if(window.myPrices[ind].price < $(this).find(".priceHere").text()) {
                    $(this).find(".priceHere").removeClass("text-danger");
                    $(this).find(".priceHere").addClass("text-success");
                }
                if(window.myPrices[ind].price > $(this).find(".priceHere").text()) {
                    $(this).find(".priceHere").removeClass("text-success");
                    $(this).find(".priceHere").addClass("text-danger");
                }
